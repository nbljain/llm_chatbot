...
from src.config import (
    DATABASE_URL, DB_TYPE, DATABRICKS_SERVER_HOSTNAME, 
    DATABRICKS_HTTP_PATH, DATABRICKS_TOKEN, DATABRICKS_CATALOG, DATABRICKS_SCHEMA
)
...
if DB_TYPE == "databricks":
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            logger.info(f"Successfully connected to Databricks at {DATABRICKS_SERVER_HOSTNAME}")
            break
...
def get_table_names() -> List[str]:
    ...
    if DB_TYPE == "databricks":
        with databricks_connection.cursor() as cursor:
            ...
            tables = [row[1] for row in result]  # Table name is usually in the second column
    else:
...
def get_table_schema(table_name: str) -> Dict[str, str]:
    ...
    if DB_TYPE == "databricks":
        with databricks_connection.cursor() as cursor:
            ...
            cursor.execute(f"DESCRIBE {DATABRICKS_CATALOG}.{DATABRICKS_SCHEMA}.{table_name}")
            result = cursor.fetchall()
            
            # Parse the result to extract column names and types
            schema = {}
            for row in result:
                if len(row) >= 2 and row[0] and row[1]:  # Ensure we have column name and type
...
def execute_sql_query(query: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if "syntax error" in error_message.lower() or "sqlexception" in error_message.lower():
        ...
        elif "column" in error_message.lower() and ("not found" in error_message.lower() or "does not exist" in error_message.lower()):
