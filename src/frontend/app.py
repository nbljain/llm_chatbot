...
with st.sidebar:
    ...
    if "error" in schema_response:
        st.error(f"Database connection failed: {schema_response.get('error', 'Unknown error')}")
    elif "db_schema" in schema_response:
        db_schema = schema_response.get("db_schema", {})
        
        if db_schema:
            # Filter out system tables like sqlite_sequence
            user_tables = {k: v for k, v in db_schema.items() if not k.startswith('sqlite_')}
            ...
            with st.spinner("Processing your question..."):
                ...
                if response.get("success") == True:
                    ...
                    if explanation:
                        ...
                        streamed_text = ""
                        for i in range(0, len(explanation), 5):  # Stream 5 characters at a time
