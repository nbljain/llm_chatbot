...
from src.backend.nlp import get_table_schema_string, generate_sql_query, generate_answer
...
def test_get_table_schema_string(monkeypatch):
    ...
    monkeypatch.setattr("src.backend.nlp.get_all_table_schemas", mock_get_all_table_schemas)
...
def test_generate_sql_query(mock_get_llm, monkeypatch):
    ...
    mock_chain.invoke.return_value = "SELECT * FROM employees WHERE department = 'Engineering'"
    mock_get_llm.return_value = mock_chain
    
    # Mock the schema string function
    monkeypatch.setattr("src.backend.nlp.get_table_schema_string", lambda: "Table schema here")
